...
if (!(AuthAdminCookie() || AuthModeratorCookie()))
{
	...
	ForumMessage("<b>You are not authorized to open this thread.</b><p>If you feel you should have access to open this thread, check to make sure you have logged in properly.");
...
fwrite($fpTemp, "$thread_title|$forum|$sticky|\n");
...
foreach ($threadindex as $i => $line)
{
	...
	{
		fwrite($fpTemp, "$thread_title|$original_membername|$info_membername|$info_timeposted|$numofposts|$thread|$sticky||\n");
	}
}
fclose($fpTemp);
